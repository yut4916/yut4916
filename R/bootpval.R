...
#' @param conf.level number between 0 and 1 (1 - alpha)
#' @param iter number; integer of bootstrap iterations
#' @param mu0 number; null hypothesis for mu
#' @param test string; "two", "upper", or "lower", indicating test tails
#'
#' @return list; p-value, tcalc, n, x, test, and conf.int
#' @export
#'
#' @examples x=rnorm(30); bootpval(x)
bootpval <- function(x, conf.level=0.95, iter=3000, mu0=0, test="two"){
  ...
  for(i in 1:iter){ # for loop - the loop will go around iter times
    ...
    #sampling from y cbind -- column bind -- binds the vectors together by columns

    xrs.mat<-cbind(xrs.mat,sample(x,n,replace=TRUE))
    #sampling from x cbind -- column bind -- binds the vectors together by columns
  }
